...
var_results_stage4_3 <- HGSOC_only[, c(15, 3:12 )] %>%
  ...
  filter(p_value < 0.05)
var_results_stage4_3 #GRB7, ZFPL1

#stage 4 vs 2, hgsoc
table(HGSOC_only$Stage4, useNA ="a")
var_results_stage4_2 <- HGSOC_only[, c(15, 3:12 )] %>%
  filter(Stage4 != "III")%>%
  pivot_longer(cols = -Stage4, names_to = "variable", values_to = "value") %>%
  group_by(variable) %>%
  summarise(
...
# GRB7 and ZFPL1 not same variances 2 vs 3  - welch
...
sjudents.test_stage <- Stage_table %>%
  filter(variable %in% c("EXO1", "RAD50", "PPT2","PKP3", "CDCA5", "ZFPL1", "VPS33B", "GRB7"))%>%
  group_by(variable) %>%
  t_test(value ~ Stage4,
         p.adjust.method = "BH", 
...
sjudents.test_stage #RAD50ND VPS33B, ZFPL1, GRB7 2 vs 4 
#Welch: GRB7 2 vs 3 and 4 vs 3, ZFPL1 4 vs 3
welch.test_stage <- Stage_table %>%
  filter(variable %in% c( "GRB7", "ZFPL1"))%>%
  group_by(variable) %>%
  t_test(value ~ Stage4,
         p.adjust.method = "BH", 
  ...
  mutate(across(c(p.adj), ~ format(., scientific = FALSE)))  # Format p-values to remove scientific notation
welch.test_stage #NONE - ZFPL1 both are equal variances, see students test
...
each.vs.ref_sig_stage <- tibble::tribble(
    ...
    "II",   "IV", 0.002, -2.5, "ZFPL1", #stjudent's
    "II",   "IV", 0.042, -1, "GRB7" #stjudent's
  )
# 3 digits after the ,
